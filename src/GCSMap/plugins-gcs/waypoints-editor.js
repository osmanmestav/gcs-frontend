...
window.addEventListener('WaypointsCleared', function () { selectedWaypointIndices = []; }, false);

function selectWaypoints(indices) {
	var selected = {};
	selectedWaypointIndices.forEach(function(index) { selected[index] = true; });
...
instantContextMenu.addItem("Loiter here", (e)=> { csharp.instantCommandIssue("LoiterUnlimited",e.coordinates.latitude,e.coordinates.longitude,plane.altitude,1,0); });
