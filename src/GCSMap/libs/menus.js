...
function ContextMenu(items) {
    ...
    if (items)
        items.forEach((i) => {
            this.addItem(i.title, i.handler);
        });
...
function PulldownMenu(title, items) {
    ...
    this.addItem = function (title, handler, options) {
        ...
        if (Array.isArray(options)) {
            ...
            el.setOptions = (options) => {
                el.selectElement.empty();
                options.forEach(o => el.selectElement.append($("<option></option>").attr("value", o.value).text(o.title)))
            };
            el.setOptions(options);
    ...
    if (items)
        items.forEach((i) => {
            this.addItem(i.title, i.handler, i.options);
        });
}
